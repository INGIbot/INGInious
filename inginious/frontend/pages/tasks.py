...
class BaseTaskPage(object):
    ...
    def set_selected_submission(self, course, task, submissionid):
        ...
        if task.is_group_task() and not is_staff:
            group = self.database.aggregations.find_one(
                {"courseid": course.get_id(), "groups.students": self.user_manager.session_username()},
        ...
        if students == submission["username"]:
            self.database.user_tasks.update_many(
                {"courseid": course.get_id(), "taskid": task.get_id(), "username": {"$in": students}},
    ...
    def GET(self, courseid, taskid, isLTI):
        ...
        try:
            ...
            tasks = OrderedDict((tid, t) for tid, t in tasks.items() if self.user_manager.task_is_visible_by_user(course, t, username, isLTI))
            task = tasks[taskid]
    ...
    def POST(self, courseid, taskid, isLTI):
        ...
        if "@action" in userinput and userinput["@action"] == "submit":
            # Verify rights
            if not self.user_manager.task_can_user_submit(course, task, username, isLTI):
            ...
            try:
                submissionid, oldsubids = self.submission_manager.add_job(course, task, userinput, debug)
                web.header('Content-Type', 'application/json')
